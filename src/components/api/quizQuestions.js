

var quizQuestions = [
	{
		index: 1,
		question: 'In what year was Sega Genesis released in North America?',
		answers:[
			{type: true, content: '1989'},
			{type: false, content: '1999'},
			{type: false, content: '1975'},
			{type: false, content: '1991'},
		]
	},

	{
		index: 2,
		question: 'Which of the following video games takes place in a dystopian underwater city called Rapture?',
		answers: [
		{type: true, content:'Bioshock' } ,
		{type: false, content: 'Half-Life'},
		{type: false, content: 'God Of War'},
		{type: false, content: 'Fallout 3'},
		]
	},

	{
		index: 3,
		question: 'What Nintendo system was released after the N64 and before the Wii?',
		answers:[
		{type: true, content:'Gamecube'},
		{type: false, content:'Nintendo 128'},
		{type: false, content: 'Virtual Boy'},
		{type: false, content: 'Super Nintendo'},
		]
	},

	{
		index: 4,
		question: 'Honda, Dhalsim and Chun Li are all characters from what video game series?',
		answers:[
		{type: true, content: 'Street Fighter'},
		{type: false, content: 'Teenage Mutant Ninja Turtles'},
		{type: false, content: 'Battletoads'},
		{type: false, content: 'Mortal Kombat'},
		]
	},
	

	{
		index: 5,
		question: 'What color is the ring of death on an XBOX that signifies a hardware failure?',
		answers:[
		{type: true, content: 'Red'},
		{type: false, content: 'Blue'},
		{type: false, content: 'Yellow'},
		{type: false, content: 'Green'},
		]
	},

	{
		index: 6,
		question:
			'What classic beat-em-up game featured brothers Billy Lee and Jimmy (also nicknamed Spike and Hammer)?',
			answers: [
		{type: true, content: 'Double Dragon'},
		{type: false, content: 'Smash Brothers'},
		{type: false, content: 'Ninja Gaiden'},
		{type: false, content: 'Snow Brothers'} ,
			]
	},

	{
		index: 7,
		question: 'How many bits was the Super Nintendo Entertainment System?',
		answers: [
		{type: true, content: '16'},
		{type: false, content: '8'},
		{type: false, content: '128'},
		{type: false, content:'64' },
		]
	},

	{
		index: 8,
		question: 'What character do you play as in The Legend Of Zelda?',
		answers: [
		{type: true, content: 'Link'},
		{type: false, content: 'Gandolf'},
		{type: false, content: 'Chimmy'},
		{type: false, content: 'Peter'},
		]
	},

	{
		index: 8,
		question: 'What 64-bit Sega system was a predecessor to the PlayStation and Nintendo 64?',
		answers:[
		{type: true, content: 'Saturn'},
		{type: false, content: 'Dreamcast'},
		{type: false, content: 'Commodore 128'},
		{type: false, content: '3DO'},
	]
	},

	{
		index: 9,
		question: 'The game Grand Theft Auto was released primarily for what gaming system?',
		answers: [
		{type: true, content: 'PlayStation'},
		{type: false, content: 'Dreamcast'},
		{type: false, content: 'XBOX'},
		{type: false, content:'NES'},
		]
	},

	{
		index: 10,
		question:
		'Which James Bond film was made into a game for the Nintendo 64 console and later for the Wii console?',
		answers: [
		{type: true, content:'Goldeneye'},
		{type: false, content: 'Thunderball'},
		{type: false, content: 'Moonraker'},
		{type: false, content:'Goldfinger'},
		]
	},

	{
		index: 11,
		question:
		'Which popular video game features an ex-Special Forces operator named Jack Carver, who is stranded in Micronesia?',
		answers:[
		{type: true, content:  'Far Cry'},
		{type: false, content: 'Halo 2'},
		{type: false, content: 'Left 4 Dead'},
		{type: false, content: 'Max Payne'},
		]
	},
];

export default quizQuestions;
